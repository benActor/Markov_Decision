...
class Node:
    ...
    def set_transitions(self, d_type, board, circular=False):
        ...
        if d_type != "risky_dice":
            for tran in Node.rem_duplicate(transitions[d_type]):
    ...
    def fill_state(self, via_node, draw):
        ...
        if via_node.n_type == 4:
            s += [{"initial_pos": self.pos, "draw": draw, "pos": i, "via": via_node.pos,
                   "prob": Action.prob(self, "risky_dice", draw)/15} for i in range(15)]
        elif via_node.n_type == 0:
            s += [{"initial_pos": self.pos, "draw": draw, "pos": via_node.pos, "via": None,
                   "prob": Action.prob(self, "risky_dice", draw)}]
        else:
    ...
    def risky_dice_s(self, d_type, board, circular):
        ...
        if self.pos in [7, 8, 9]:
            ...
            for draw in Action.ROLL[d_type]:
                if not draw:
                    s += [{"initial_pos": self.pos, "draw": draw, "pos": self.pos, "via": None,
                           "prob": Action.prob(self, "risky_dice", draw)}]
                else:
                    if self.pos + draw <= 9:
                        via_node = board[self.pos + draw]
                        s += self.fill_state(via_node, draw)
                    elif self.pos + draw > 9 and not circular:
    ...
    def a_in_l(l, a):
        if not l:
            return False
        elif Node.equal(a, l[0]):
            return True
        else:
            return Node.a_in_l(l[1:], a)

    @staticmethod
    def combine_prob(similar_trans):
        return {"initial_pos": similar_trans[0]["initial_pos"],
                "draw": similar_trans[0]["draw"],
                "pos": similar_trans[0]["pos"],
                "prob": sum(trans["prob"] for trans in similar_trans)}
    ...
    def rem_duplicate(tran_list):
        res = Node.gsm(tran_list, tran_list[0])
        for x in tran_list[1:]:
            res = Node.gsm(res, x)
        return res
